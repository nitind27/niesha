...
export async function sendStaffWelcomeEmail(params: {
  ...
  const fromRaw = process.env.EMAIL_FROM?.trim() || process.env.SMTP_USER!.trim()
  const subject = `Welcome to ${escapeHtml(params.schoolName)} — Your staff account is ready`
...
<body style="margin:0;padding:0;font-family:system-ui,-apple-system,sans-serif;background:#f8fafc;color:#1e293b;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:36px 40px;">
            ...
            <p style="margin:0 0 24px;font-size:14px;color:#64748b;line-height:1.6;">
              Your staff account has been created at <strong>${escapeHtml(params.schoolName)}</strong> as <strong>${escapeHtml(params.designation)}</strong>.
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;border-radius:10px;margin-bottom:28px;">
              <tr><td style="padding:20px 24px;">
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="padding:6px 0;font-size:14px;color:#64748b;">Employee ID</td>
                    <td style="padding:6px 0;font-size:14px;font-weight:700;color:#1e293b;text-align:right;">${escapeHtml(params.employeeId)}</td>
                  </tr>
                  <tr>
                    <td style="padding:6px 0;font-size:14px;color:#64748b;">Designation</td>
                    <td style="padding:6px 0;font-size:14px;font-weight:600;color:#1e293b;text-align:right;">${escapeHtml(params.designation)}</td>
                  </tr>
                  <tr>
                    <td style="padding:6px 0;font-size:14px;color:#64748b;">Login Email</td>
                    <td style="padding:6px 0;font-size:14px;font-weight:600;color:#1e293b;text-align:right;">${escapeHtml(params.to)}</td>
                  ...
                  <tr>
                    <td style="padding:6px 0;font-size:14px;color:#64748b;">Password</td>
                    <td style="padding:6px 0;font-size:14px;font-weight:700;color:#10b981;text-align:right;font-family:monospace;">${escapeHtml(params.defaultPassword)}</td>
                  </tr>
                </table>
              </td></tr>
            </table>
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr><td align="center">
                <a href="${escapeHtml(params.loginUrl)}" style="display:inline-block;background:linear-gradient(135deg,#10b981,#059669);color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;padding:13px 32px;border-radius:10px;">
  ...
  const text = `Welcome ${params.firstName} ${params.lastName}!\n\nYour staff account at ${params.schoolName} is ready.\n\nEmployee ID: ${params.employeeId}\nDesignation: ${params.designation}\nLogin Email: ${params.to}\nPassword: ${params.defaultPassword}\n\nLogin: ${params.loginUrl}\n\nPlease change your password after first login.`
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e)
    console.error("[mail] sendStaffWelcomeEmail failed:", msg)
    return { sent: false, reason: "send_failed", message: process.env.NODE_ENV === "development" ? msg : undefined }
  }
}
