  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Upcoming and past exams for your class</p>
      </div>

      {isLoading && <div className="h-48 rounded-2xl bg-muted animate-pulse" />}

      {!isLoading && exams.length === 0 && (
        <div className="text-center py-20 text-muted-foreground">No exams scheduled yet.</div>
      )}
            ...
            <CardContent className="pt-4 pb-4">
              <div className="flex items-start justify-between gap-4">
                  ...
                  <div>
                    ...
                    {exam.class && (
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {exam.class.name}{exam.section ? ` · ${exam.section.name}` : ""}
                      </p>
                    )}
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  ...
                  {exam.totalMarks && (
                    <p className="text-xs text-muted-foreground mt-0.5">Total: {exam.totalMarks} marks</p>
                  )}
                </div>
              </div>
            </CardContent>
