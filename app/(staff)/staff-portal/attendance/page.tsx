...
export default function StaffAttendancePage() {
  ...
  useEffect(() => {
    api.get("/staff/me").then((r) => setAttendance(r.data.staff?.attendance ?? [])).catch(() => {}).finally(() => setIsLoading(false))
  }, [])
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2"><Calendar className="h-4 w-4 text-primary" /> Attendance Log</CardTitle>
            <Badge variant={pct >= 75 ? "default" : "destructive"}>{pct}% overall</Badge>
          </div>
        </CardHeader>
        <CardContent>
          {isLoading && <div className="h-32 bg-muted animate-pulse rounded-xl" />}
          {!isLoading && attendance.length === 0 && <p className="text-center text-muted-foreground py-8">No attendance records yet.</p>}
